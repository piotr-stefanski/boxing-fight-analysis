...
def main(video_path, draw_on_image=False) -> None:
    ...
    prev_time = 0
    recently_found_fight_timestamp = 0
    processed_frames = 0
    ...
    for i in range(video_frames):
        ...
        count_of_all_persons, count_persons_on_ring, len_boxers_on_ring, is_clash, distance_between_boxers = utils.is_clash_in_image(
            net, img, draw_rectangle_on_persons=draw_on_image)

        if is_clash:
            recently_found_fight_timestamp = time.time()

        if is_clash or time.time() - recently_found_fight_timestamp < 3:
            if draw_on_image:
                draw_text_on_image(img, 'CLASH', (int(video_resolution[0] / 2), 350))
